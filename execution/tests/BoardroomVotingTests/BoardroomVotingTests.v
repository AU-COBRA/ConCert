...
From ConCert.Execution.QCTests Require Import
  TestUtils ChainPrinters TraceGens BoardroomVotingPrinters BoardroomVotingGens.
...
Definition boardroom_chain : ChainBuilder :=
  let chain : ChainBuilder := lcb_initial AddrSize in
  let dep := build_act creator (create_deployment 0 (boardroom_voting hash_func) deploy_setup) in
  (* do chain <- TraceGens.add_block chain [] ; *)
  unpack_result (
    TraceGens.add_block chain [dep]
  ).


Definition boardroom_caddr := A 128.

Definition voters : FMap Address (Z * Z * Z) :=
  Eval native_compute in 
    let keys := zip (zip pks sks) rks in
    FMap.of_list (zip addrs keys).
    
Definition voters_signup : FMap Address ((list positive -> positive) -> Msg) :=
  Eval native_compute in 
    let msgs := map (fun '(sk, pk, i) => fun hf => make_signup_msg hf sk 5 i)
                             (zip (zip sks pks) (seq 0 (length sks))) in
    FMap.of_list (zip addrs msgs).

Module TestInfo <: BoardroomVotingGensInfo.
  Definition contract_addr := boardroom_caddr.
  ...
  Definition gAccountWithout (ws : list Address) :=
    ...
    elems_opt addrs.
  Definition hash_func := hash_func.
                                  (* public, private, reconstructed key *)
  Definition voters := voters.
  Definition voters_signup := voters_signup.
End TestInfo.
Module MG := BoardroomVotingGens.BoardroomVotingGens TestInfo. Import MG.
From ConCert.Execution.QCTests Require Import
  SerializablePrinters.

Instance gBoardroomVotingChainBuilder : GenSized ChainBuilder := 
{|
  arbitrarySized := gBoardroomVotingTrace boardroom_chain
|}.
Instance shrinkChainBuilder : Shrink ChainBuilder := {| shrink a := [a] |}.

Global Instance G_Monad : Monads.Monad G :=
{| 
  ret _ t := returnGen t; 
  bind _ _ mt f := bindGen mt f 
|}.
Global Instance G_Opt_Monad : Monads.Monad (fun X => G (option X)) :=
{| 
  ret _ t := returnGenSome t; 
  bind _ _ mt f := bindGenOpt mt f 
|}.

(* Definition gMsg : G (option Action) := 
  gBoardroomVotingMsg . *)
...
Set Printing All.
Definition what : GOpt BoardroomVoting.Msg := liftM (fun msg => deserialize BoardroomVoting.Msg _ (serialize BoardroomVoting.Msg _ msg)) gMsg.
...
{|
  show v := match @deserialize BoardroomVoting.Msg _ v with
  | Some v => show v
...
Definition gBoardroomVotingMsg : G (option Action) :=
  let call caller (m : Msg) := returnGenSome ( 
    build_act caller (act_call boardroom_caddr 0 (@serialize BoardroomVoting.Msg _ m))) in
  ...
  backtrack [
    ...
    | tally_votes.
    *)
    (1%nat, bindGenOpt (sampleFMapOpt voters_signup) (fun '(voter, h) => 
            let msg : Msg := h hash_func in
                        call voter msg
            )) ;
    (1%nat, do msg <- arbitrary ; (call zero_address) msg )
  ].

Local Open Scope list.
(* Open Scope broom. *)
Definition dummy_hash_func (l : list positive) : positive :=
  N.succ_pos (fold_right (fun p a => N.add (Npos p) a) 1%N l).
