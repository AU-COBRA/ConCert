...
Section Instances.
  ...
  #[export]
  Program Instance result_serializable `{Serializable A}
                                       `{Serializable B}
                                       : Serializable (ResultMonad.result A B) :=
    {| serialize s := serialize
                        match s with
                        | ResultMonad.Err e => inl e
                        | ResultMonad.Ok s => inr s
                        end;
      deserialize s := do s <- deserialize s;
                        match s : B + A with
                        | inl e => Some (ResultMonad.Err e)
                        | inr s => Some (ResultMonad.Ok s)
                        end; |}.
  Next Obligation.
    intros.
    cbn.
  ...
  #[export]
  Program Instance ser_byte : Serializable byte :=
    {| serialize b := serialize (Byte.to_nat b);
      deserialize n := do n' <- deserialize n; Byte.of_nat n'; |}.
  Next Obligation.
    intros.
    cbn.
    rewrite deserialize_serialize.
...
Section Countable.
  ...
  (* Low priority to always pick 'direct' Serializable instances first *)
  Local Program Instance countable_serializable {A : Type} `{countable.Countable A} :
    Serializable A | 1000 :=
    {| serialize v := serialize (countable.encode v);
       deserialize v := do p <- (deserialize v : option positive); countable.decode p |}.
  Next Obligation.
    intros A Heq Hcountable x.
    cbn.
    rewrite deserialize_serialize.
    apply countable.decode_encode.
  Qed.
End Countable.
...
Section RightInverse.
  ...
  Lemma deserialize_byte_right_inverse : forall x (y : byte),
    deserialize x = Some y ->
    x = serialize y.
  Proof.
    intros * deser_some.
    apply deserialize_nat_right_inverse.
    cbn in *.
    destruct extract_ser_value;
      try discriminate.
    destruct (i <? 0)%Z;
      try discriminate.
    apply to_of_nat in deser_some as <-.
    reflexivity.
  Qed.
...
Module DeriveSerializable.
  Local Ltac make_serializer_case ty :=
    match ty with
    | ?T1 -> ?T2 =>
  ...
  Local Ltac make_serializer_aux term tag :=
  ...
  Local Ltac make_serializer eliminator :=
  ...
  Local Ltac make_deserializer_case ty :=
    match ty with
    | ?T1 -> ?T2 =>
  ...
  Local Ltac make_deserializer_aux ctors rty :=
    match ctors with
    | (?ctor, ?tl) =>
...
End DeriveSerializable.

Export DeriveSerializable.
