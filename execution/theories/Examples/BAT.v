...
Instance state_settable : Settable _ :=
	settable! build_state <total_supply; balances; allowances; fundDeposit; batFundDeposit; 
...
Global Instance msg_serializable : Serializable Msg :=
  Derive Serializable Msg_rect <transfer, transfer_from, approve, create_tokens, finalize, refund>.
...
Section BATtoEIP20Token.
(* here we make sure that the functionality related to transfer, transfer_from, and approve uses the logic from the EIP20Token contract *)
(* So essentially we're doing 'extension' as understood in mainstream object-oriented languages. *)
...
Definition init (chain : Chain)
	...
	Some {|
		...
		total_supply := setup.(_batFund);
		balances := FMap.add setup.(_batFundDeposit) setup.(_batFund) FMap.empty;
...
Definition try_create_tokens sender (sender_payload : Amount) current_slot state :=
 (* early return if funding is finalized, funding period hasn't started yet, or funding period is over *)
	...
	let checkedSupply := state.(total_supply) + tokens in
...
Definition try_refund sender current_slot state := 
	...
	do _ <- returnIf (state.(isFinalized) || (Nat.leb current_slot state.(fundingEnd)) || (state.(tokenCreationMin) <=? state.(total_supply))) ;
	(* Don't allow the the batFundDeposit account to refund *)
	do _ <- returnIf (address_eqb sender state.(batFundDeposit)) ;
	do sender_bats <- FMap.find sender state.(balances) ;
	do _ <- returnIf (sender_bats =? 0) ;
	let new_total_supply := state.(total_supply) - sender_bats in
...
Definition try_finalize sender current_slot contract_balance state :=
	...
	*)
	do _ <- returnIf (state.(isFinalized) || 
									 ...
									 (state.(total_supply) <? state.(tokenCreationMin))) ;
	do _ <- returnIf ((Nat.leb current_slot state.(fundingEnd)) && negb (state.(total_supply) =? state.(tokenCreationCap))) ;
...
Proof. repeat intro; solve_contract_proper.
	destruct (_ =? _)%address; auto; 
	destruct (negb _); auto; 
	destruct (orb _ _); auto; 
	simpl; rewrite H; simpl; 
	rewrite (account_balance_eq x y); try reflexivity; apply H.
Qed.
