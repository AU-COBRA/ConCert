...
Module BuildUtils.
  ...
  Definition receiver_can_receive_transfer `{ChainBase} (bstate : ChainState) act_body :=
    match act_body with
  ...
  Axiom deployable_address_decidable : forall `{ChainBase} bstate wc setup act_origin act_from amount,
    reachable bstate ->
  ...
  Open Scope Z_scope.
  Lemma action_evaluation_decidable `{ChainBase} : forall bstate act,
    reachable bstate ->
  ...
  Definition produces_no_new_acts `{ChainBase} act : Prop :=
  ...
  *)
  Lemma empty_queue `{ChainBase} : forall bstate (P : ChainState -> Prop),
    reachable bstate ->
  ...
  Proof.
    ...
    - (* Case: queue contains at least one action,
      ...
      apply list_relations.list.Forall_cons_1 in acts_from_account as [act_from_a acts_from_account].
      apply list_relations.list.Forall_cons_1 in no_new_acts as [no_new_acts_from_a no_new_acts].
  ...
  Lemma add_block `{ChainBase} : forall bstate reward creator acts slot_incr,
    reachable bstate ->
  ...
  Lemma forward_time_exact `{ChainBase} : forall bstate reward creator slot,
    reachable bstate ->
    chain_state_queue bstate = [] ->
    address_is_contract creator = false ->
    reward >= 0 ->
  ...
  Lemma forward_time `{ChainBase} : forall bstate reward creator slot,
    reachable bstate ->
    chain_state_queue bstate = [] ->
    address_is_contract creator = false ->
    reward >= 0 ->
  ...
  Lemma evaluate_action `{ChainBase} : forall {Setup Msg State Error : Type}
                                `{Serializable Setup}
                                `{Serializable Msg}
                                `{Serializable State}
                                `{Serializable Error}
    ...
    BlockchainBase.receive contract (transfer_balance from caddr amount bstate)
  ...
  Lemma evaluate_transfer `{ChainBase} : forall bstate origin from to amount acts,
    reachable bstate ->
    chain_state_queue bstate = {| act_from := from;
                                  act_origin := origin;
  ...
  Lemma discard_invalid_action `{ChainBase} : forall bstate act acts,
    reachable bstate ->
  ...
  Lemma permute_queue `{ChainBase} : forall bstate acts acts_permuted,
    reachable bstate ->
    chain_state_queue bstate = acts ->
  ...
  Lemma deploy_contract `{ChainBase} : forall {Setup Msg State Error : Type}
                                `{Serializable Setup}
                                `{Serializable Msg}
                                `{Serializable State}
                                `{Serializable Error}
