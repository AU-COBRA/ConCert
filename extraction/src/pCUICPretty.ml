...
| Coq_tInd (ind, _) ->
  ...
  (match lookup_ind_decl (fst_ctx _UU03a3_) i k with
   | Checked a -> let (_, body) = a in substring O (S O) (ind_name body)
   | TypeError _ -> 'X'::[])
| _ -> 'U'::[]
...
| Coq_tInd (ind, u) ->
  ...
  (match lookup_ind_decl (fst_ctx _UU03a3_) i k with
   | Checked a ->
     let (_, oib) = a in append (ind_name oib) (print_universe_instance u)
   | TypeError _ ->
     append
       ('U'::('n'::('b'::('o'::('u'::('n'::('d'::('I'::('n'::('d'::('('::[])))))))))))
       (append
...
| Coq_tCase (indn, p, t0, brs) ->
  ...
  (match lookup_ind_decl (fst_ctx _UU03a3_) mind i with
   | Checked a ->
     let (_, oib) = a in
     (match p with
      | Coq_tLambda (na, _, b) ->
