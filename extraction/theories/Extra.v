...
Fixpoint erasable_program (name : kername) : TemplateMonad program :=
  cst <- tmQuoteConstant name false;;
  let body := cst_body cst in
  match body with
  | None => tmFail ("cannot get body of constant " ++ string_of_kername name)
  | Some body =>
    ...
    | S tries =>
      ...
      | EnvError Σ (IllFormedDecl _ e)  =>
        ...
        | UndeclaredConstructor c _ =>
          ...
          let env := List.filter (fun '(n, _) => negb (eq_kername n name)%string) env in
          aux tries (env ++ [(name, InductiveDecl ind)])%list
        | _ =>
