...
Import ExAst.
Local Opaque flag_of_type erase_type.
...
erase_constant_decl cst wt with flag_of_type [] (P.cst_type cst) _ := {
  | Checked {| is_sort := left issort |} with inspect (P.cst_body cst) := {
    | exist (Some body) eq :=
  ...
  | Checked {| is_sort := right _ |} with erase_type [] []%vector (P.cst_type cst) _ [] := {
    | Ok ety with inspect (P.cst_body cst) := {
      | exist (Some body) eq with erase_func Σ wfextΣ [] body _ := {
        | Checked eb => Ok (inl {| cst_type := ety; cst_body := Some eb |});
        | TypeError e => Err (EraseBodyError e)
        };
      | exist None _ := Ok (inl {| cst_type := ety; cst_body := None |})
      };
    | Err e => Err (EraseTypeError e)
    };
  | TypeError e => Err (EraseBodyError e)
  }.
Next Obligation.
