...
let bind_option_state (a b c : unit) (m : option A) (f : A -> state option) : state option =
match m with
    Some a -> f a
...
let test_receive (ctx : contractCallContext) (state : state) (maybe_msg :  (msg) option) = let sender = ctx_from ctx in 
